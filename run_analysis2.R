...
# re_tidy flags rebuild of "tidy_data2.csv" file.
# Both flags false by default, script will attempt to use existing local files.
run_analysis2 <- function(return_tidy_data = FALSE, re_prepare = FALSE, 
                         re_tidy = FALSE) {
    ...
    filename_tidy_data <- "tidy_data2.csv"  
    ...
    if(file.exists(filename_tidy_data) == FALSE | re_tidy == TRUE) {
        re_tidy = TRUE
        message("Info: File tidy_data2.csv will be rebuilt.")
    }
    ...
    if(re_prepare == TRUE & re_tidy == FALSE) {
        re_tidy = TRUE
        message("Info: File tidy_data2.csv will therefore also be rebuilt.")
    }
    ...
    if(re_tidy) {
        ...
        tidy_df <- tidy_data2(tbl_df(read.csv(filename_prepared_data)))
        
        # Write a CSV with the tidy data.
        write.csv(tidy_df, file.path(filename_tidy_data), row.names = FALSE)
        
        message("Info: File tidy_data2.csv has been rebuilt.")
    }
    
    if(re_prepare == FALSE & re_tidy == FALSE) {
        message("Warning: No processing was performed.")
        message("Info: prepared_data.csv and tidy_data2.csv already exist.")
