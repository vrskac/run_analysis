...
The following call to the **run\_analysis** function will re-prepare and re-tidy the data before writing it out to textfile and also returning it as a data frame to the caller. The call has been wrapped within a call to str() for presentation purposes.
